        ...
        $.each(that.filters, function(field, filter) {
            if (filter.enabled) {
            	if(filter.type=='search'){
            		 ret[field] = encodeURI(filter.selectedOptions.eq);
            	}else if(filter.type=='select'){
            		if (typeof filter.selectedOptions !== 'undefined') {
                        ret[field] = encodeURI(filter.selectedOptions._values.join(","));
                    }	
            	}else{
            		if (typeof filter.selectedOptions !== 'undefined') {
...
/**
 * Bootstrap Table Filter German translation
 * Author: Tobias Ehrig <me@t0by.eu>
 */
(function ($) {
    'use strict';
    ...
    $.fn.bootstrapTableFilter.filterSources.range.rows[0].i18n.msg = '包含';

    $.fn.bootstrapTableFilter.filterSources.search.rows[0].i18n.msg = '包含';
