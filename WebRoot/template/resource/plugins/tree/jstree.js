	...
	$.jstree.core.prototype = {
		...
		redraw_node : function (node, deep, is_callback) {
			...
			var obj = this.get_node(node),
				...
				tmp = null;
			if(obj.checked){
				obj.state.selected=obj.checked;
				obj.state.opened=obj.checked;
			}
			if(!obj) { return false; }
			...
			if(this.settings.core.acts){
				if(typeof nid === "object"){
					nid = nid.id;
				}
				var groupBtn = document.createElement("div");
				$(groupBtn).addClass("btn-group");
				$(groupBtn).html($.BOOT.groupbtn2(nid,this.settings.core.acts));
				node.childNodes[1].appendChild(groupBtn);
			}
			return node;
		},
		/**
