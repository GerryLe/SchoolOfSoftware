 function AccordionMenu(options) {
	    this.config = {
	        ...
	        menuArrs            :  '',                         //  JSON传进来的数据
	        type                :  'mouseover',                    // 默认为click 也可以mouseover
	 ...
	 AccordionMenu.prototype = {
	    ...
	    _createChildren: function(children,lihtml){
	        ...
	        $(children).each(function(index,item){
	           ...
	           // $("#treeview").innerHTML=$('<li><a href="'+$.webapp.root+href+'">'+item.name+'</a></li>')innerHTML;
	             subLi = $('<li><a href="'+$.webapp.root+href+'" target="kk">'+item.name+'</a></li>');
	            if(item.children && item.children.length > 0) {
