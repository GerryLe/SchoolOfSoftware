...
$.BOOT.alert = function(json, custom) {
	if (custom) {
		json["confirmButtonText"] = $.BOOT.jscache["ok"];
		json["cancelButtonText"] = $.BOOT.jscache["concel"];
	}
...
$.BOOT.notice = function() {
	var href = $.webapp.root + "/admin/system/message/no_list.do";
	$.post(href, {}, function(result) {
		var ul = $("#notice-id");
		$("#notice-id-count").html(result[0].count);
		ul.append("<li class='header'>你有" + result[0].count + "条未读消息</li>");
		ul.append("<li><ul class='menu'>");
		for (var i = 1; i < result.length; i++) {
			var li = "<li><a href='#'>";
			li += "<i class='fa fa-circle text-aqua'></i>";
			li += result[i].title + "</a></li>";
			ul.append(li);
		}
		ul.append("</ul></li>");
		var href = "/admin/system/message/my_message_main_UI.do";
		ul.append("<li class='footer'><a href='#' onclick=\"$.BOOT.page('"
				+ href + "');\">显示所有</a></li>");
	}, 'json');
};
/**
...
$.BOOT.savePermits = function($tree, $value, type) {
	...
	$.post($.webapp.root + "/admin/system/acl/no_grantPermits.do", {
		...
		if (result.status) {
			$.BOOT.alert("success", result.msg);
		} else {
			$.BOOT.alert("warning", result.msg);
		}
	}, 'json');
};
/**
...
$.BOOT.initmenu = function() {
			...
			.on(
					...
					function() {
						...
						if (url != "null") {
							...
							html += '<div class="btn-group"><a class="dropdown-toggle" data-toggle="dropdown">';
							...
							html = '<div class="tab-pane fade in active" id="'
									+ id + '">';
							html += '<iframe class="iframe-item" width="'
	...
	$(document).on("click", ".simple-tabs-li .fa-times", function() {
		var obj = $(this).parents(".simple-tabs-li");
		var id = obj.attr("data-id");
		$("#" + id).remove();
		obj.remove();
...
$.BOOT.navmenu = function(id, func) {
	...
	$.post(href, {
		...
		var aTag = function(menu, pull) {
			var a = "<a href='javascript:;' class='nav-menu' data-name='"
					+ menu.text + "' data-id='" + menu.id + "' data-url='"
					+ menu.href + "'";
			a += " > <i class='fa ";
			a += menu.iconCls;
			a += "' style='color:";
...
$.BOOT.logout = function(page) {
	$.post($.webapp.root + "/system/login/logout.do", null, function(result) {
		if (result.status) {
			...
			$.BOOT.alert("warning", '安全退出失败，请联系管理员！');
		}
	}, 'json');
};
/**
 * 分组按钮
 */
$.BOOT.groupbtn1 = function(id, json, setting) {
	...
	var p = "";
	if (setting && setting.right == true) {
		p = "dropdown-menu-right";
	}
	html += "<ul class='dropdown-menu " + p + "'>";
	for ( var d in json) {
		var data = json[d];
		if (data.type == "s") {
...
$.BOOT.groupbtn = function(id, json, setting) {
	return "<div class='btn-group'>" + $.BOOT.groupbtn1(id, json, setting)
			+ "</div>";
};
/**
 * 获取值
...
$.BOOT.table = function(id, href, opts) {
	...
	var ps = {
		...
		paginationInfo : getOptionValue(opts.paginationInfo, false),// 卡片显示
		...
		onDblClickRow : getOptionValue(opts.onDblClickRow, null),
