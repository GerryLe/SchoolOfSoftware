package com.rosense.basic.util.date;
...
public class DateUtils {
	...
	public static int formatDateNum(String format) {
		SimpleDateFormat sdf = new SimpleDateFormat(format);
		return LangUtils.toInt(sdf.format(new Date()), 0);
	}

	public static int formatDateNum(Date date, String format) {
		SimpleDateFormat sdf = new SimpleDateFormat(format);
		return LangUtils.toInt(sdf.format(date), 0);
	}
	...
	public static Date formatDate(String date, final String format) {
		SimpleDateFormat sdf = new SimpleDateFormat(format);
		try {
			return sdf.parse(date);
		} catch (ParseException e) {
	 ...
	 * 根据指定日期判断上午下午
	 */
	public static Integer getAmPm(Date date) {
		Calendar ca = Calendar.getInstance();
		ca.setTime(date);
		return ca.get(Calendar.AM_PM);
	}
	 ...
	 * @return
	 */
	public static long diffDate(Date d1, Date d2, int type) {
		Calendar cal = Calendar.getInstance();
		cal.setTime(d1);
		long time1 = cal.getTimeInMillis();
		cal.setTime(d2);
		...
		long diff = 0;
		if (type == 0) {//天
			diff = (time2 - time1) / (1000 * 3600 * 24);
		} else if (type == 1) {//小时
			diff = (time2 - time1) / (1000 * 3600);
		} else {//分钟
			diff = (time2 - time1) / (1000 * 60);
		}
	...
	public static String convertDateToCron(String datetime) {
		try {
			...
			sc.setTime(dt);
			return sc.get(Calendar.MINUTE) + " " + sc.get(Calendar.SECOND) + " " + sc.get(Calendar.HOUR_OF_DAY) + " " + sc.get(Calendar.DAY_OF_MONTH)
					+ " " + (sc.get(Calendar.MONTH) + 1) + " ? " + (sc.get(Calendar.YEAR)) + "-" + (sc.get(Calendar.YEAR));
		} catch (ParseException e) {
	 ...
	 * 计算工作日
	 */
	public static int workDays(int ym, String workDays) {
		int count = 0;
		Calendar cal = Calendar.getInstance();
		cal.setTime(formatDate(String.valueOf(ym), "yyyyMM"));
		...
		while (cal.get(Calendar.MONTH) < month) {
			...
			if (workDays.contains(String.valueOf(day))) {
				count++;
			}
	...
	public static String formatHours(double hours) {
		int day = (int) (hours / 24);//计算差多少天
		int hour = (int) (hours % 24);//计算差多少天
		String v = String.valueOf(hours);
		int idx = v.indexOf(".");
		if (idx != -1) {
		...
		int minute = (int) (LangUtils.toInt(v, 0) * 6);//计算差多少天
		return day + "天" + hour + "小时" + minute + "分钟";
	}

	public static void main(String[] args) throws ParseException {
		//		long hour = hours%24/60;//计算差多少小时
		...
		System.out.println(convertDateToCron("2014-09-12 10:05:10"));*/
		//System.out.println(onLineTimeInfoDay("10:10:31","hh:mm:ss"));
         
		System.out.println(dateDiffStr("2014-9-12 10:10", "2014-9-12 21:15",DEFAULT_DATE_HH_MM_PATTERN));
	}

	/**
	 * 把日期格式化为数字
	 */
	public static int toDateNum(Date date, String format) {
		return LangUtils.toInt(formatDate(date, format), 0);
	}

	/**
	 * 0时0分0秒
	 */
	public static final String CN_FMT_DAY = "{0,number,integer}天{1,number,integer}时{2,number,integer}分";
	...
	public static final String onLineTimeInfoDay(String time,String format) {
		try {
			final long baseTime = DateUtils.formatDate(time, format).getTime();
			...
			final long m = ma / 60;
			// h+"时"+m+"分"+sa+"秒"
			return MessageFormat.format(CN_FMT_DAY, new Object[] { d, h, m });
		} catch (Exception e) {
		}
		return MessageFormat.format(CN_FMT_DAY, new Object[] { 0, 0, 0 });
	}
	...
	public static final String nowDateTime(String format) {
